...
Expected<void> LiteRtCompiledModelT::RegisterBuffer(
  ...
  if (buffer_requires_cpu_sync) {
    ...
    bool buffer_is_cpu_compatible =
        buffer->buffer_type() == kLiteRtTensorBufferTypeHostMemory ||
        buffer->is_opencl_memory() || buffer->buffer_type() == kLiteRtTensorBufferTypeD3D12Buffer;
#if defined(__ANDROID__)
...
#endif
    // LITERT_LOG(LITERT_ERROR, "======buffer_is_cpu_compatible %d, %d", buffer_is_cpu_compatible, buffer->buffer_type());
