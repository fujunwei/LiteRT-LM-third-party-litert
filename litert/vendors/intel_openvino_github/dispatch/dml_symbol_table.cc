...
#include "litert/vendors/intel_openvino/dispatch/dml_symbol_table.h"
...
// The DxCore symbols.
LATE_BINDING_SYMBOL_TABLE_DEFINE_BEGIN(DXCoreSymbolTable, "DXCore.dll")
#define X(sym) LATE_BINDING_SYMBOL_TABLE_DEFINE_ENTRY(DXCoreSymbolTable, sym)
DXCore_SYMBOLS_LIST
#undef X
LATE_BINDING_SYMBOL_TABLE_DEFINE_END(DXCoreSymbolTable)

DXCoreSymbolTable* GetDXCoreSymbolTable() {
  static DXCoreSymbolTable* dxcore_symbol_table = new DXCoreSymbolTable();
  dxcore_symbol_table->Load();
  return dxcore_symbol_table;
}
